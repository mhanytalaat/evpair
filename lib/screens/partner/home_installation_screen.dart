...
///   - "Request Service": submit a categorized request (station,
///     adaptor, cable, maintenance, other) and track status of past
///     requests. Name/phone/area are pre-filled from your account and
///     car profile where possible - see _ServiceRequestTabState.initState.
///   - "Equipment": browse the borrow/buy catalog (cables, adaptors,
///     home stations), filterable by category, each shown with a photo
///     when the listing has one (falls back to a category icon
///     otherwise) - see _EquipmentTab. Each listing also shows who it's
///     from - EVPair stock, a specific brand (e.g. "PowerX"), or an
///     individual installation partner - via EquipmentOwnerType.
///
...
// ===========================================================================
// Tab 1: Request Service (categorized: station / adaptor / cable /
// maintenance / other)
// ===========================================================================
...
class _ServiceRequestTabState extends State<_ServiceRequestTab> {
  ...
  void initState() {
    ...
    // Pre-fill everything we already know about this driver, instead of
    // asking them to retype it every time:
    //   - Name and phone come straight from their EVPair account
    //     (AuthService), same source Account Settings uses.
    //   - Area defaults to their car's registered compound/community
    //     (CarProfile.community), since that's the most likely place
    //     they'd want a home service performed.
    //   - City/Area both also fall back to whatever they used on their
    //     MOST RECENT request of any category, so a returning driver
    //     doesn't retype their address every single time.
    ...
    _cityCtrl = TextEditingController(text: mostRecent?.city ?? '');
    _areaCtrl = TextEditingController(text: mostRecent?.area ?? app.car?.community ?? '');
  }
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Pre-filled from account/car profile, but editable -
                  // see initState above for exactly where each default
                  // comes from.
                  TextFormField(
...
// ===========================================================================
// Tab 2: Equipment marketplace - categorized (Station / Adaptor / Cable
// / Other) with an owner-type badge (Partner / Brand / EVPair) and a
// product PHOTO on each listing (falls back to a category icon when a
// listing has no imageUrl set yet - see EquipmentListing.imageUrl).
// ===========================================================================
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        // Category filter chips - mirrors the Partner Jobs open-requests
        // filter, so browsing equipment feels consistent with browsing
        // service requests elsewhere in the app.
        SizedBox(
          height: 34,
          child: ListView(
